...
function Gauge({ label = 'Great!', fill = 80 }: { label?: string; fill?: number }) {
  const len = Math.PI * 30 // semicircle arc length ≈ 94
  return (
    <svg width="74" height="42" viewBox="0 0 74 42" aria-hidden>
      <path d="M7 38 A30 30 0 0 1 67 38" fill="none" stroke="#e6e6e6" strokeWidth="7" strokeLinecap="round" />
      <path
      ...
      />
      <text x="37" y="36" textAnchor="middle" fontSize="13" fontWeight="800" fill="#1a1a1a" style={{ fontFamily: 'inherit' }}>
        {label}
      </text>
    </svg>
  )
...
const eyebrowIcons: Record<string, ReactNode> = {
  ...
  pitch: (
    <svg width="20" height="20" viewBox="0 0 20 20" fill="none" stroke="currentColor" strokeWidth="1.7" strokeLinejoin="round">
...
function DownArrow() {
  return (
    <svg
      ...
      className="mx-auto my-2 text-ink-muted"
      aria-hidden
    >
  ...
  return (
    <div className="grid items-center gap-8 rounded-3xl bg-white/[0.025] p-6 md:grid-cols-2 md:p-10">
      <div className={reverse ? 'md:order-2' : ''}>
        <p className="mb-4 flex items-center gap-2 text-sm font-bold uppercase tracking-[0.18em] text-yellow">
          <span className="text-yellow">{eyebrowIcons[icon]}</span>
          {eyebrow}
        </p>
        <h3 className="text-[clamp(1.6rem,3.5vw,2.3rem)] font-extrabold leading-[1.15] tracking-[-0.02em] text-ink-strong">
          {heading}
        </h3>
        <p className="mt-4 max-w-md text-lg font-light leading-relaxed text-ink">{body}</p>
      </div>
...
const HL = ({ children }: { children: ReactNode }) => (
  <span className="font-semibold text-ink-strong underline decoration-2 underline-offset-2">{children}</span>
)
  ...
  return (
    <div className="rounded-xl bg-white p-4 text-left text-[#1a1a1a] shadow-2xl">
      <p className="mb-3 text-sm font-bold">
        ...
        <span className="font-normal text-gray-500">Dental Clinics in Austin, TX</span>
      </p>
  ...
  return (
    <div className="rounded-xl bg-white p-5 text-left text-[#1a1a1a] shadow-2xl">
      <div className="flex items-center justify-between border-b border-gray-100 pb-2">
        ...
        <span className="rounded border border-gray-200 px-2 py-0.5 text-xs text-gray-500">Not Contacted ▾</span>
      </div>
  ...
  return (
    <div className="space-y-3 rounded-xl bg-white p-5 text-left text-[#1a1a1a] shadow-2xl">
      ...
      <p className="rounded border border-gray-200 px-3 py-2 text-sm leading-relaxed text-gray-600">
        Hi Jordan — noticed Lumen Dental still takes bookings by phone. A small AI
        scheduler could cut no-shows and free up your front desk…
      </p>
      <button className="ml-auto flex items-center gap-1.5 rounded bg-yellow px-3 py-1.5 text-sm font-bold text-on-light">
        ✦ Generate Different
      </button>
    </div>
  )
...
function RadarEmailMock() {
  return (
    <div className="rounded-xl bg-white p-4 text-left text-[#1a1a1a] shadow-2xl">
      <p className="text-sm font-semibold">32 New Prospects Found — Dental Clinics in Austin, TX</p>
      ...
      <div className="rounded-lg bg-[#0e0e12] p-4 text-center text-white">
        ...
        <p className="font-extrabold text-yellow">32 Great Prospects This Week</p>
      </div>
      <div className="mt-3 rounded-lg border border-gray-200 p-3">
        ...
        <div className="mt-2 flex flex-wrap items-center gap-1.5">
          {['No CRM', 'Manual booking', 'Legacy stack'].map((t) => (
            <span key={t} className="rounded bg-red-50 px-2 py-0.5 text-[10px] font-medium text-red-600">
              🔥 {t}
            </span>
          ))}
  ...
  return (
    <section className="mx-auto max-w-6xl space-y-2 px-4 py-16 sm:px-6">
      <Row
        ...
        heading={
          <>
            Find more than just contact details. <span className="underline decoration-yellow decoration-4 underline-offset-4">Find opportunities.</span>
          </>
        }
      ...
      <Row
        ...
        body={
          <>
            Find <HL>manual workflows</HL> and <HL>missed automations</HL> to make sharp,
            relevant pitches. This <span className="rounded bg-ink-strong px-1.5 text-on-light">boosts open and reply rates</span>.
          </>
        }
      ...
      <Row
        ...
        heading={
          <>
            Personalized outreach in <span className="rounded bg-ink-strong px-1.5 text-on-light">one click</span>
          </>
        }
        body={
          <>
            Generate <HL>tailored pitch emails</HL> grounded in each company's real gaps.
          </>
        }
        mock={<GeneratorMock />}
      />
      ...
      <Row
        ...
        heading={
          <>
            Get new prospects <HL>delivered to your inbox</HL> each month on autopilot
          </>
        }
        body={
          <>
            Pick your niche, set your region, and Opportunity Radar{' '}
            <span className="rounded bg-ink-strong px-1.5 text-on-light">does the prospecting for you</span> on autopilot.
          </>
        }
        mock={<RadarEmailMock />}
      />
